package com.heroesofpentaarena
...
class NoirModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    fun loadCircuit(circuitData: String, recursive: Boolean, promise: Promise): String? {
        try {
            val circuit = Circuit.fromJsonManifest(circuitData, null, recursive)
    ...
    @ReactMethod fun setupCircuit(circuitData: String, recursive: Boolean, promise: Promise) {
        Thread {
            val circuitId = loadCircuit(circuitData, recursive, promise)
            if (circuitId == null) {
                promise.reject("CIRCUIT_LOAD_FAIL", "Unable to load circuit. Please check the circuit was compiled with the correct version of Noir")
                return@Thread
            }
    ...
    @ReactMethod fun execute(inputs: ReadableMap, circuitId: String, promise: Promise) {
        Thread {
            ...
            try {
                ...
                var result: WritableMap = Arguments.createMap()
                result.putArray("witness", Arguments.fromList(witness!!.toList()))
                // result.putString("proof", proof!!.proof)
                // result.putString("vkey", proof!!.vk)
                promise.resolve(result)
            } catch (e: Exception) {
                Log.d("PROOF_GENERATION_ERROR", e.toString())
                promise.reject("PROOF_GENERATION_ERROR", "Unable to execute circuit")
            }
        }.start()
    }
    ...
    @ReactMethod fun prove(inputs: ReadableMap, circuitId: String, promise: Promise) {
        Thread {
            ...
            try {
                var proof: String? = circuit.prove(inputs.toHashMap(), "honk")
    ...
    @ReactMethod fun getVerificationKey(circuitId: String, promise: Promise) {
        Thread {
            val circuit = circuits.get(circuitId)
            if (circuit == null) {
                promise.reject("CIRCUIT_NOT_LOADED", "Circuit not loaded. Please load the circuit before generating a proof")
    ...
    @ReactMethod fun verify(proof: String, circuitId: String, promise: Promise) {
        Thread {
            ...
            try {
                var verified: Boolean? = circuit.verify(proof, null, "honk") // proofType ?: "honk")
    ...
    @ReactMethod fun verifyWithKey(proof: String, vkey: String, circuitId: String, promise: Promise) {
        Thread {
            val circuit = circuits.get(circuitId)
            if (circuit == null) {
                promise.reject("CIRCUIT_NOT_LOADED", "Circuit not loaded. Please load the circuit before verifying a proof")
            ...
            try {
                var verified: Boolean? = circuit.verify(proof, vkey, "honk")
